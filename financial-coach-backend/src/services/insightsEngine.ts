...
export async function generateInsights(userId: number): Promise<{ insights: Insight[], advice: string }> {
    // 0. Determine "Current" Date (Anchor to latest transaction)
    const maxDateStmt = db.prepare("SELECT MAX(date) as lastDate FROM transactions WHERE user_id = ?");
    const maxDateResult = maxDateStmt.get(userId) as { lastDate: string } | undefined;
    ...
    const totalSpendStmt = db.prepare(`
        ...
        AND date <= ?
    `);
    const totalSpendRes = totalSpendStmt.get(userId, anchorDate, anchorDate) as { total: number };
    ...
    const topCategoryStmt = db.prepare(`
        ...
        LIMIT 1
    `);
    const topCategory = topCategoryStmt.get(userId, anchorDate, anchorDate) as { category: string, total: number } | undefined;
    ...
    const largeTxStmt = db.prepare(`
        ...
        LIMIT 1
    `);
    const largestTx = largeTxStmt.get(userId, anchorDate, anchorDate) as Transaction | undefined;
    ...
    const statsStmt = db.prepare(`
        ...
        WHERE user_id = ? AND amount < 0
    `);
    const stats = statsStmt.get(userId) as { avg_amt: number, variance: number };
    ...
    const anomalyStmt = db.prepare(`
        ...
        WHERE user_id = ? AND amount < 0 AND ABS(amount) > ? AND date >= date(?, '-30 days')
        LIMIT 1
    `);
    const anomalyTx = anomalyStmt.get(userId, anomalyThreshold, anchorDate) as Transaction | undefined;
    ...
    // Data Point 6: Goals
    const goalsStmt = db.prepare('SELECT * FROM goals WHERE user_id = ? AND current_amount < target_amount');
    const activeGoals = goalsStmt.all(userId) as any[];

    // --- ML: Anomaly Detection ---
    let anomalyData = null;
    if (anomalyTx) {
        anomalyData = {
            merchant: anomalyTx.merchant,
            amount: Math.abs(anomalyTx.amount),
            threshold: anomalyThreshold,
            is_anomaly: true
        };
    }

    // --- ML: Burn Rate Analysis ---
    const burnRateStatus = calculateBurnRate(userId, anchorDate);
    ...
    const financialContext = {
        ...
        summary: {
            ...
            goals_list: activeGoals.map(g => `${g.name}: $${g.current_amount}/$${g.target_amount}`),
        },
        ml_findings: {
            anomaly: anomalyData,
            burn_rate: burnRateStatus ? {
                type: burnRateStatus.type, // 'alert' or 'achievement'
                projected_total: burnRateStatus.impact_amount ? (burnRateStatus.impact_amount + (burnRateStatus.type === 'alert' ? 1000 : 0)) : 0, // Approximate back-calc or just pass raw data next time.
                // Actually, let's pass the raw Insight object from the helper, it has good data
                trend: burnRateStatus.type === 'alert' ? 'High Spending' : 'Saving Well',
                details: burnRateStatus.message
            } : null
        }
    };

    // Check transaction volume - Skip LLM if negligible data
    const countStmt = db.prepare(`
        ...
        AND date <= ?
    `);
    const countRes = countStmt.get(userId, anchorDate, anchorDate) as { count: number };

    // Only call AI if we have meaningful data (>5 transactions)
    let analysis: { insights: Insight[], advice: string } = { insights: [], advice: "" };

    if (countRes.count > 5) {
        console.log(`Generating AI analysis for user ${userId} (${countRes.count} txns)...`);
        console.log("Context:", JSON.stringify(financialContext, null, 2));
        analysis = await generateCombinedAnalysis(financialContext);
    } else {
        console.log(`Skipping AI analysis for user ${userId} - Insufficient data (${countRes.count} txns).`);
        analysis.advice = "Upload more transactions to unlock your AI financial coach!";
    }

    // Fallback static insights
    if (analysis.insights.length === 0) {
        analysis.insights = generateStaticInsights(userId, anchorDate);
    }

    return analysis;
}

function calculateBurnRate(userId: number, anchorDate: string): Insight | null {
    ...
    const dailyData = stmt.all(userId, startOfMonth, anchorDate) as { date: string, daily_total: number }[];
    ...
    `);
    const avgRes = avgStmt.get(userId, startOfMonth) as { avg_spend: number };
...
function generateStaticInsights(userId: number, anchorDate: string): Insight[] {
    const insights: Insight[] = [];

    // 1. Large Purchase
    const largeTxStmt = db.prepare(`SELECT * FROM transactions WHERE user_id = ? AND amount < -200 AND strftime('%Y-%m', date) = strftime('%Y-%m', ?) ORDER BY amount ASC LIMIT 1`);
    const largeTx = largeTxStmt.get(userId, anchorDate) as Transaction | undefined;
    ...
    const burnRate = calculateBurnRate(userId, anchorDate);
    if (burnRate) {
